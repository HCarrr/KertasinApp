...
class InvoicePenjualanController extends GetxController {
  ...
  void onInit() async {
    super.onInit();
    print('Initializing InvoicePenjualanController with user ID: $currentUserId');
        ...
        .listen(
          (QuerySnapshot snapshot) {
            ...
            for (var change in snapshot.docChanges) {
              ...
              print('Doc change: ${change.type}, ID: ${change.doc.id}, nomorInvoice: ${data['nomorInvoice']}');
            }
            isLoading.value = false;
            final fetchedInvoices = snapshot.docs.map((doc) {
              final data = doc.data() as Map<String, dynamic>;
              final nomorInvoice = data['nomorInvoice']?.toString() ?? 'INV-SALE-N/A';
              return {
                'id': doc.id,
  ...
  String formatHarga(dynamic harga) {
    ...
    return numberFormat.format(harga is num ? harga.floor() : 0).replaceAll(',', '.');
  }
